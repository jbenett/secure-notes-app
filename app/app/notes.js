...
// Get email and temporary auth token from local storage
let email = localStorage.getItem("email");
let temp_auth_token = localStorage.getItem("temp_auth_token");

// Send a GET request to backend to notes for given email and auth token
$.get("http://localhost:3000/notes", {email: email, temp_auth_token: temp_auth_token})
.done(function(result) {
	let maxLength = 200;

	// Modify DOM to add retrieved notes
